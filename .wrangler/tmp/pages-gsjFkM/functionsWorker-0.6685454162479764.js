...
var onRequestGet = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const tours = (toursRes.results || []).map((t) => {
      let visited = [];
      try {
      ...
      const tourTickets = documents.filter((doc) => doc.tourId === t.id);
      return {
        ...t,
    ...
    return new Response(
      JSON.stringify({
        ...
        settings: (settingsRes.results || []).reduce((acc, curr) => {
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  const db = context.env.DB;
  try {
    const doc = await context.request.json();
    if (!doc.id || !doc.title) {
...
// ../.wrangler/tmp/pages-gsjFkM/functionsRoutes-0.7770737416455237.mjs
var routes = [
  {
    routePath: "/api/data",
    mountPath: "/api",
